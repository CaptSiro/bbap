...
Each type of database is required to follow the specification and implement its own type of connection interface including
Object-Relation Mapping (ORM) for querying data.

\subsubsection*{Asset Loading}
The system is designed for server-side-rendering as well as granular UI streaming.
...
The importer is used to generate one blob of all required assets.
The blob is sent to the frontend where it is parsed and incorporated.
...
table listing of records from a collection, creation of new records, and editing existing records.
